...
dd_xitong() {
		...
		dd_xitong_4() {
		  ...
		  while true; do
			...
			echo -e "${gl_hui}스크립트 지원에 대해 MollyLau와 bin456789에게 감사드립니다!${gl_bai} "
			echo "------------------------"
...
linux_panel() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}31.  ${gl_bai}StirlingPDF 도구 모음${gl_kjlan}32.  ${gl_bai}drawio 무료 온라인 차트 작성 소프트웨어${gl_huang}★${gl_bai}"
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}25.  ${gl_bai}TG-bot 시스템 모니터링 및 조기경보${gl_kjlan}26.  ${gl_bai}OpenSSH 고위험 취약점 복구(Xiuyuan)"
...
EOF
		  ...
		  66)
			  ...
			  echo -e "5. 모든 포트를 엽니다"
			  echo -e "6. 켜기${gl_huang}BBR${gl_bai}加速"
			  ...
			  case "$choice" in
				[Yy])
				  ...
				  echo "------------------------------------------------"
				  bbr_on
				  echo -e "[${gl_lv}OK${gl_bai}] 6/10. 열려 있는${gl_huang}BBR${gl_bai}加速"
